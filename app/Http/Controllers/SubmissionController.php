...
class SubmissionController extends Controller
{
     ...
     */
    public function attachFile(Request $request, $courseId, $courseWorkId, $submissionId)
    {
        $user = Auth::user();
        $classroom = $this->getGoogleService($user);

        $driveFileId = $request->input('driveFileId');

        if (!$driveFileId) {
            return response()->json(['message' => 'driveFileId is required'], 400);
        }

        try {
            $attachment = new \Google_Service_Classroom_ModifyAttachmentRequest([
                'addAttachments' => [
                    new \Google_Service_Classroom_Attachment([
            ...
            $classroom->courses_courseWork_studentSubmissions
                ->modifyAttachments($courseId, $courseWorkId, $submissionId, $attachment);
    ...
    /**
     * Turn in a submission (after attaching file or editing)
     */
    public function turnInWithAttachment(Request $request, $courseId, $courseWorkId, $submissionId)
    {
        $user = Auth::user();
        $classroom = $this->getGoogleService($user);
        ...
        try {
            ...
            return response()->json(['message' => 'Submission turned in successfully']);
        } catch (\Google_Service_Exception $e) {
            return response()->json([
                'message' => 'Failed to turn in submission',
                'error' => $e->getMessage(),
